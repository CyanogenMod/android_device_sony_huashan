...
/* ===================================================================== */
/* === Module Constants === */
#ifndef LEDS_UNIT_COUNT
    #define LEDS_UNIT_COUNT 1
    #undef LIGHT_LEDS_ADVANCED
#endif
...
static unsigned int
brightness_apply_gamma(unsigned int brightness) {
    double floatbrt = (unsigned double) brightness;
    floatbrt /= 255.0;
    ...
    floatbrt *= 255.0;
    brightness = (unsigned int) floatbrt;
                      ...
                      struct light_state_t const* state)
{
    ...
    ALOGV("set_light_leds_locked : %d %d %d - delayOn : %d, delayOff : %d - Update : %d/%d - Brightness : %d - LEDs Mode : %d - Mode : %d (Not. 1 / Bat. 2)\n",
          rgb[0], rgb[1], rgb[2], delayOn, delayOff, leds_rgb_update, leds_program_update, 255, leds_modes, g_leds_state);
    (void)dev;
    return 0;
}
