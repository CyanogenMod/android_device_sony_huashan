...
inline static status_t finish_flatten_binder(
    const sp<IBinder>& __attribute__((unused)) binder, const flat_binder_object& flat, Parcel* out)
{
    return out->writeObject(flat, false);
}

status_t flatten_binder(const sp<ProcessState>& __attribute__((unused)) proc,
...
inline static status_t finish_unflatten_binder(
    BpBinder* __attribute__((unused)) proxy,
    const flat_binder_object& __attribute__((unused)) flat,
    const Parcel& __attribute__((unused)) in)
{
    return NO_ERROR;
}
...
void Parcel::remove(size_t __attribute__((unused)) start, size_t __attribute__((unused)) amt)
{
...
String16 Parcel::readString16() const
{
    ...
    ALOGE("Reading a NULL string not supported here (%ud).", len);
    return String16();
}
...
const flat_binder_object* Parcel::readObject(bool nullMetaData) const
{
    ...
    if ((DPOS+sizeof(flat_binder_object)) <= mDataSize) {
        ...
        if (N > 0) {
            ...
            if (OBJS[opos] == DPOS) {
                // Found it!
                ALOGV("Parcel found obj %p at index %d with forward search, %ud",
                     this, DPOS, opos);
            ...
            if (OBJS[opos] == DPOS) {
                // Found it!
                ALOGV("Parcel found obj %p at index %d with backward search, %ud",
                     this, DPOS, opos);
                mNextObjectHint = opos+1;
                ALOGV("readObject Setting data pos of %p to %d\n", this, mDataPos);
                return obj;
...
void Parcel::print(TextOutput& to, uint32_t __attribute__((unused)) flags) const
{
    to << "Parcel(";
